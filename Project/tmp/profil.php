<html>
   ...
   <body>
        ...
        <!-- Type de la requete -->
        <div>
            <?php echo "<h1>Mon profil -- Adh&eacute;rent n&deg;".$_GET['id']." </h1>"; ?>
        </div>
    ...
    <tbody>
			...
			<?php
			include "connect.php"; 	
            $requete = "CALL info_adherent(".$_GET['id'].");";
            // Si l'execution est reussie... 
            if($res = $connection->query($requete))
            // ... on récupère un tableau stockant le résultat 
				while ($var =  $res->fetch_assoc()) {
					echo "\t".'<tr><td>'.$var['nom'].'</td>';
		...
		if (!empty($_POST['kilometres'])) {
			...
			echo "<h3>Distance parcourue par semaine :</h3>";
    ...
    <tbody>
		...
		if (!empty($_POST['reabonnement'])) {
            ...
            if($res = $connection->query($requete)){
				$id = $res->fetch_assoc();
				header("Location: https://cdeplanne001.vvvpedago.enseirb-matmeca.fr/tmp/profil.php?id=".$id['new_id_adherent']."&reabonnement=1");
				$connection->close();
				exit();
			}
