<?php
	...
	$requete="CREATE TRIGGER utilisations_velos_before
       ...
       IF @date_fin IS NOT NULL AND @date_fin < NEW.date_debut
       THEN
...
END;";
	...
	$requete="CREATE PROCEDURE ajout_fin_utilisation(IN p_id_adherent INT, IN p_id_station_fin INT)
BEGIN
...
END;";
	...
	$requete="CREATE PROCEDURE nbr_km_parcourus_semaine (IN p_id_adherent INT)
BEGIN
...
BEGIN
SELECT id_station_fin, count(*) as nbr_utilisations FROM utilisations WHERE id_adherent = p_id_adherent AND id_station_fin IS NOT NULL GROUP BY id_station_fin ORDER BY nbr_utilisations;
END;";
	$connection->query($requete);
...
END;";
	...
	$requete="CREATE PROCEDURE fin_adhesion(IN p_id_adherent INT)
BEGIN
...
END;";
	...
	$requete="CREATE PROCEDURE se_reabonner(IN p_id_adherent INT)
BEGIN
       IF p_id_adherent NOT IN (SELECT id_adherent FROM adherents) 
THEN
        SIGNAL SQLSTATE '45000'
...
END;";
	...
	$requete="CREATE PROCEDURE taux_reabonnement()
BEGIN
...
END;";
	...
	$requete="CREATE PROCEDURE recharge_batterie(IN p_id_velo INT, IN p_batterie INT)
BEGIN
       IF p_id_velo NOT IN (SELECT id_velo FROM velos) 
THEN
        SIGNAL SQLSTATE '45000'
...
END;";
	...
	$requete="CREATE PROCEDURE rank_velos_station(IN p_id_station INT)
BEGIN
       IF p_id_station NOT IN (SELECT id_station FROM stations) 
THEN
        SIGNAL SQLSTATE '45000'
...
END;";
	...
	$requete="CREATE PROCEDURE avg_nbr_usager_velo_jour(IN p_id_velo INT)
BEGIN
...
END;";
	...
	$requete="CREATE PROCEDURE avg_nbr_utilisations_jour(IN p_day DATE)
BEGIN
...
END;";
	...
	$requete="CREATE PROCEDURE delete_utilisation(IN p_id_utilisation INT)
BEGIN
...
END;";
	...
	$requete="CREATE PROCEDURE delete_adherent(IN p_id_adherent INT)
BEGIN
      IF p_id_adherent NOT IN (SELECT id_adherent FROM adherents) 
      THEN
        SIGNAL SQLSTATE '45000'
...
END;";
	...
	$requete="CREATE PROCEDURE ajout_fin_utilisation_peuplement(IN p_id_adherent INT, IN p_id_station_fin INT,  IN p_date_fin DATETIME)
BEGIN
        ...
        /** MISE A JOUR TABLE UTILISATIONS KM PARCOURU + date_fin **/
        UPDATE utilisations SET kilometrage_parcouru = @dist WHERE id_utilisation = @id_utilisation;
        UPDATE utilisations SET date_fin = p_date_fin WHERE id_utilisation = @id_utilisation;
...
END;";
	...
	$requete="CREATE PROCEDURE ajout_debut_utilisation_peuplement(IN p_id_velo INT, IN p_id_adherent INT, IN p_date_debut DATETIME)
BEGIN
        SELECT id_station INTO @p_id_station FROM velos WHERE id_velo = p_id_velo;
        INSERT INTO utilisations(id_velo, id_adherent, id_station_debut, date_debut, kilometrage_parcouru)
        VALUES
        (p_id_velo, p_id_adherent, @p_id_station, p_date_debut, 0);
END;";
	$connection->query($requete);
	
	
	$requete="CREATE VIEW stations_bornes AS SELECT id_station, nombre_bornes FROM stations;";
	$connection->query($requete);
	
	
	$requete="CREATE VIEW nbr_places_dispos_stations AS SELECT id_station, nombre_bornes - nbr_velos from stations_bornes INNER JOIN nbr_velos_dispos_stations USING(id_station);";
	$connection->query($requete);
	
	
	
	$requete="CREATE VIEW nbr_renew0 AS SELECT count(*) as nbr_renew FROM adherents GROUP BY id_personne;";
	$connection->query($requete);

	$requete="CREATE VIEW nbr_renew1 as SELECT nbr_renew FROM nbr_renew0 WHERE nbr_renew > 1;";
	$connection->query($requete);


	$requete="CREATE VIEW nbr_pers_renew as SELECT count(nbr_renew) as nbr_pers_renew FROM nbr_renew1;";
	$connection->query($requete);
