<html>
   ...
   <body>
        ...
        <form method = "post">
            <p>
				<input type="submit" class = 'button' value=""  />
				<label>Liste des adh&eacute;rents qui utilisent au moins deux v&eacute;los pour un jour donn&eacute; -- date : </label>
        ...
        <form method = "post">
            <p>
				...
				<input type="submit" class = 'button' value=""  />
				<label>Listes des v&eacute;los en cours d'utilisation</label>
            </p>
        </form>
...
<?php
    ...
    <tbody>
		...
		if (!empty($_POST['nbr_velos'])) {
			...
			echo "<h3>Nombre de v&eacute;los :</h3>";
			include "connect.php"; 	
            $requete = "SELECT count(id_velo) FROM velos;";
            // Si l'execution est reussie... 
            if($res = $connection->query($requete))
            // ... on récupère un tableau stockant le résultat 
				while ($station =  $res->fetch_assoc()) {
					echo $station['count(id_velo)']."<br>";
				}
		...
		if (!empty($_POST['velo_utilisation'])) {
			...
			echo "<h3>Listes des v&eacute;lo en cours d'utilisation :</h3>";
			include "connect.php"; 	
			echo "<h4>Id_velo</h4>";
		...
		if (!empty($_POST['id_velo'])) {
			...
			echo "<h3>Informations sur le v&eacute;lo ".$_POST['id_velo']." :</h3>";
