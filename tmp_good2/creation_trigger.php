...
END;";
	...
	$requete="CREATE VIEW nbr_pers_renew as SELECT count(nbr_renew) as nbr_pers_renew FROM (SELECT nbr_renew FROM (SELECT count(*) as nbr_renew FROM adherents GROUP BY id_personne) as al WHERE nbr_renew > 1) as a;";
	$connection->query($requete);
...
END;";
	...
	$requete="CREATE PROCEDURE avg_nbr_utilisations_jour(IN p_day VARCHAR(100))
BEGIN
...
END;";
	...
	$connection->close();
	echo "Triggers, proc&eacute;dures et vues cr&eacute;&eacute;es";
?>
